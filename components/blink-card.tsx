...
export function BlinkCard({ data }: { data: ActionGetResponse }) {
  return (
    <Card className="w-full shadow-2xl border-none font-[sans-serif]">
      <CardContent className="p-4">
        <img
          src={data.icon}
          alt="action-image"
          className="w-full aspect-square object-cover rounded-sm overflow-hidden bg-muted"
        />
        ...
        <div className="mt-4 flex flex-col">
          <div className="mb-2 flex items-center gap-2">
            <Link
              href="#"
              className="inline-flex items-center truncate text-muted-foreground hover:text-foreground text-xs transition-all hover:underline"
            >
              <svg
            ...
            >
              <div className="bg-muted text-muted-foreground hover:text-foreground transition-all flex items-center justify-center rounded-full aspect-square p-1">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="13"
                  height="13"
          ...
          <p className="mb-0.5 leading-none font-bold text-primary">{data.title}</p>
          <p className="mb-4 text-xs text-secondary-foreground">{data.description}</p>
          {!data.links || data.links.actions.length == 0 ? (
            <Button className="w-full">{data.title}</Button>
          ) : (
              ...
              {data.links.actions
                ...
                .map((action, index) => (
                  <div
                    ...
                    className="flex items-center gap-2 rounded-md border border-input-primary transition-colors focus-within:border-input-checked motion-reduce:transition-none"
                  >
                    <input
